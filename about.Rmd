...
ggplot() + 
  geom_polygon(data = US, aes(x=long, y = lat, group = group), fill="grey", alpha=0.3) + geom_point(data=df, aes(x=Long, y=Lat, size=cnt, color=cnt)) + coord_map() + ylim(20,50) + xlim(-130,-60) + labs(title="Where I've called home",color="years") + scale_size_continuous(guide = FALSE) + theme(legend.position = c(0.9,0.4))
